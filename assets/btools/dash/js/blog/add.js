...
$(function () {
  ...
  $('#idb').on('select2:select', function (e) {
    var idb = e.params.data.id;
    $('#myIdb').val(idb);
    getLabelBlog(idb);
  });
  ...
  $('.formPost').submit(function(e){
    ...
    $.ajax({
      ...
      success: function (d) {
        ...
        .then(() => window.location.href = '/a/blog/b/posts/' + mid);
      },
      error: function (e) {
        $.unblockUI()
  ...
  function getLabelBlog(idf) {
    $.blockUI({ message: itemLoader, css: { backgroundColor: 'transparent', border: '0'}, overlayCSS: { backgroundColor:"#fff", opacity:.8 } });
    $.getJSON(`/api/getLabelBlogger?idb=${idf}`, function (d) {
      $.unblockUI();
      $("tags").remove();
      new Tagify(document.querySelector("#lblf"), { whitelist: d.data, maxTags: 10, dropdown: { maxItems: 20, classname: "", enabled: 0, closeOnSelect: !1 } });
    });
  }
