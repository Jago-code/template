...
document.addEventListener("DOMContentLoaded", function (e) {
  const fv = FormValidation.formValidation(document.querySelector("#formPost"), {
    fields: {
        pid: { validators: { notEmpty: { message: "Please select a Page!" } } },
...
$(function () {
  $('#pageSelect').on('change', function() {
    const myVal = this.value.split('|--|');
    dt_basic.ajax.url(`?type=json&pid=${myVal[0]}&token=${myVal[1]}`).load();
  });
    ...
    dt_basic = dt_basic_table.DataTable({
      ...
      columnDefs: [
        ...
        },
        {
          targets: 1,
          render: function (data, type, full, meta) {
            return full['message'] ? full['message'].substring(0, 18) : full['story'] ? full['story'] : '-';
          }
        },
        {
          targets: -2,
          render: function (data, type, full, meta) {
            return moment(full['created_time']).format('LLL');
          }
        },
        {
          // Actions
  ...
  window.deletPost = async(idp) => {
      ...
      .then(function (e) {
        if (e.value) {
          $.blockUI({ message: itemLoader, css: { backgroundColor: 'transparent', border: '0'}, overlayCSS: { backgroundColor:"#fff", opacity:.8 } });
          $.ajax({
            data: { idp, token: $('#pageSelect').val().split('|--|')[1] },
            url: '/a/social/delete/facebook',
            type: "POST",
            success: function (d) {
              $.unblockUI();
              Swal.fire({ title: "Good job!", text: d.msg, icon:"success", customClass:{ confirmButton:"btn btn-primary" }, buttonsStyling: !1 })
	...
	$('#btnLogged').click(function(){
	  Swal.fire({ text: "Are you sure you want to log out of your facebook account?", icon: "warning", showCancelButton: !0,confirmButtonText: "Yes", customClass: { confirmButton: "btn btn-primary me-2", cancelButton: "btn btn-label-secondary" }, buttonsStyling: !1 })
      .then(function (e) {
        if (e.value) {
