...
$(function($) {
    $('ul.navigation-main li a').each(function() {
...
$.post(`/api/notif`, function(d){
  $('#lsntf').html('');
  $('#lsc').html(d.system.length);
  $('#lsc2').html(`${d.system.length} Baru`);
  d.system.forEach(ld => {
    $('#lsntf').append(`<a class="d-flex" href="#">
    <div class="list-item d-flex align-items-start">
        <div class="me-1"><div class="avatar bg-light-${ld.icon == 'info' ? 'info' : 'warning'}"><div class="avatar-content">${feather.icons[ld.icon].toSvg({ class: 'avatar-icon' })}</div></div>
        </div>
        <div class="list-item-body flex-grow-1">
            <p class="media-heading">${ld.title}</p><small class="notification-text"> ${ld.desc}</small>
        </div>
    </div>
  </a>`);
  });
